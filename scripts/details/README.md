...
| `check-install-script-parity.py` | Asserts the helper functions copied between the standalone scripts under [`scripts/install/`](../install/) and [`scripts/checks/`](../checks/) are byte-identical, so the standalone guarantee does not cost silent divergence |
