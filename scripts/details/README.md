...
| [`tests/`](tests/README.md) | Image verification: asserts a built image contains the pinned versions, and that the toolchain works. Bind-mounted into the image under test |
...
python3 scripts/details/tests/verify-image.py --self-test          # the suite's own tests, offline
```
...
The pin readers - `check-dependencies-pins.py`, `render-manifest.py` and `tests/verify-image.py` -
read the manager regexes out of `renovate.json` rather than restating them, so what Renovate tracks,
what the guard enforces, what the release note lists and what the images are verified against cannot
drift apart.  
