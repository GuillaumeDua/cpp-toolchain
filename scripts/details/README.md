...
| `shared.sh` | One body per helper the scripts under [`scripts/`](../) share. The public ones cannot source it - each has to work as a single file - so they carry copies, and the internal ones source it. Also names the two things a caller defines itself, `retry_backoff_seconds` and `error()` |
| `check-install-script-parity.py` | Asserts every helper the standalone scripts under [`scripts/install/`](../install/) and [`scripts/checks/`](../checks/) copy out of `shared.sh` still matches it, so the standalone guarantee does not cost silent divergence. `--fix` rewrites the copies from it instead of reporting them |
...
`shared.sh` runs nowhere - it is sourced, or copied out of.

```bash
python3 scripts/details/check-dependencies-pins.py               # exits non-zero and reports every violation
python3 scripts/details/check-install-script-parity.py           # copies still match shared.sh (--fix rewrites them)
