...
| `render-manifest.py` | Renders those pins as the markdown "what's inside" table used for the GitHub release description, or (`--bumps-yaml`) as the `bumps:` mapping of a promotion record. `--versions` adds what the images installed and diffs it against the previous record, `--versions-dir` turns the collected files into that mapping; `--changelog` splices a merged-pull-request list into the same marked region; `--replace-region` edits a release body around those markers, refusing an unbalanced pair |
...
| `cxx-toolchain-versions.sh` | Runs *inside* a published image and reports the C++ toolchain it carries - both compilers and both standard library implementations, with their ABI levels - as `key=value` lines. Only what a pin cannot say: GCC and Clang pin a major and install from rolling apt sources, and the standard libraries have no pin at all. Composes [`gcc.sh`/`llvm.sh --list-installed`](../install/) and [`cxx-stdlibs.sh`](../checks/cxx-stdlibs.sh) |
