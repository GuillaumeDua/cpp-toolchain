...
The four Python scripts run from the **repository root** - their default paths are relative to the working directory.
...
`check-dependencies-pins.py` and `check-install-script-parity.py` are the [build gate](../../.github/workflows/docker-build.yml)'s first two steps, so running them before pushing saves a round trip.
