...
A custom build can name any triplet from `binutils.sh --list-available` - **CPU, FPU, ABI and endianness are selected by the triplet itself**, there is no separate switch.  
